...
THEANO_FLAGS=device=\$device\$devno python \$TRAIN/config.py -runtime \$RUNTIME -vocab-size \$VOCAB_SIZE -source \$SRC -target \$TRG -validate-script \$TRAIN/validate-qsub.sh
EOF
...
while true; do
    ...
    qsub -sync y -cwd -S /bin/bash -V -l mem_free=16g,gpu=1,h_rt=$RUNTIME -j y -o logs/ ./train-$SRC-$TRG.sh
